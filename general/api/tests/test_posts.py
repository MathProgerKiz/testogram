...
class PostTestCase(APITestCase):
    ...
    def test_try_to_update_other_post(self):
        ...
        # Обновляем объект поста из базы данных, чтобы проверить его текущее состояние
        post.refresh_from_db()

        # Проверяем, что поля поста не изменились
    ...
    def test_try_to_update_other_post_with_put(self):
        # Создаем пост, принадлежащий другому пользователю
        other_user = UserFactory()  # Создаем другого пользователя
        post = PostFactory(
            author=other_user,
            title="old_title",
            body="old_body",
        ...
        # Выполняем PUT запрос на обновление поста
        response = self.client.put(
            path=f"{self.url}{post.pk}/",
            data=new_data,
            format="json",
        )
        # Проверяем, что возвращен статус 403 Forbidden
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
